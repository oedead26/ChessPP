...
GameWindow::GameWindow(int intGameID)
{
    ...
    boolIsInitialClick = true;
    Move* move = new Move(ptrGame->getPreviousMove());
    previousMove = ptrGame->getMovesAsStrings().size() > 0 ? move : nullptr;
    setupGUI();
}
    ...
    if (!ptrGame->isCheckmate(colorToMove) && !ptrGame->isStalemate(colorToMove))
    {
        ...
        else
        {
            ...
            if (ptrGame->isMoveLegal(aMove))
            {
                ...
                delete previousMove;
                previousMove = new Move(initialSquare, finalSquare);
