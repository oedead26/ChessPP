...
{
	int intKingRow, intKingCol;
    for (int i = 0; i < NUM_OF_ROWS; i++)
	{
        for (int j = 0; j < NUM_OF_COLS; j++)
		{
			Piece* potentialPiece = getPieceAt(i, j);
			if (potentialPiece != nullptr)
			{
				if (potentialPiece->getColor() == color && potentialPiece->getType() == KING)
				{
					intKingRow = i;
					intKingCol = j;
				}
	...
	PieceColor opponentColor = color == WHITE ? BLACK : WHITE;
	return isTeamAttackingSquare(getSquare(intKingRow, intKingCol), opponentColor);
}
